...
The ER605 is configured for **dual-ISP load balancing with automatic failover**. No inbound port forwarding is configured at the ER605 level — all external access is handled through the cloud VPS.

### WAN Policy
| ISP | Role | Weight | Notes |
|-----|------|--------|-------|
| ISP A (Bnet)| Primary (active) | 1 | Handles the majority of traffic |
| ISP B (Ovall) | Backup/failover | 1 | Takes over if ISP A goes down |
...
| 4 | WireGuard subnet (`10.9.0.1/24`) | VPS internal | All | Any | Allow tunnel traffic |
...
## Container-Level Firewall / Network Isolation
